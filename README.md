...
### 🏋 운동 자세 교정  
**Squat Analysis**  
사용자의 스쿼트 자세를 **OpenCV와 MediaPipe**를 이용해 실시간으로 추적하고, **양쪽 무릎 관절 각도**를 기반으로 반복 횟수와 정확도를 분석합니다.  
분석된 자세 정보는 **Gemini 기반 AI 피드백 시스템**을 통해 **자세 교정 코멘트와 리포트 이미지로 시각화**되어 제공되며, **자세 개선 추적 기능**까지 함께 제공합니다.

---

### 🧬 체형 예측 및 운동 추천  
**Body & Workout**  
사용자의 성별, 연령, BMI 등을 기반으로 **랜덤 포레스트 모델**로 체형을 예측하고, **예측된 체형에 따라 운동 난이도(초급~고급)**를 구분하여 **맞춤형 추천 영상**을 제공합니다.  
또한, 운동 목적(근력 향상, 체중 감량 등)에 따른 **효과별 필터링 추천**도 지원하며, **CSV 기반 운동 영상 데이터 + Plotly 시각화**를 통해 직관적인 운동 콘텐츠를 제공합니다.

---

### 💉 질병 예측  
**Disease Risk**  
건강검진 데이터를 기반으로 **당뇨, 고혈압, 고지혈증 등의 유병 확률을 RandomForest 모델로 예측**하고, 위험도는 **막대 그래프와 툴팁 정보**를 통해 시각화됩니다.  
또한, **유전력 기반 암 위험도 분석** 및 **2D 레이더 차트, 3D 정육면체 시각화**를 통해 사용자의 **전반적인 건강 상태를 종합 분석**합니다.

---

### 🧠 우울증 예측  
**Mental Health**  
PHQ-9 설문, 수면 시간, 건강검진 데이터를 기반으로 **우울증 단계를 AI 모델이 예측(없음~심각)**하며, 결과는 **수면 연관 차트**와 함께 시각적으로 제공됩니다.  
**Gemini 챗봇이 사용자 데이터 기반 심리 코멘트를 생성**하고, **SHAP 분석을 통해 예측에 영향을 준 주요 요인**을 시각적으로 설명하여 신뢰도를 높입니다.

---

### 🏥 병원 추천  
**Hospital Match**  
사용자 건강검진 데이터를 기반으로 **질병 유무(당뇨, 고지혈증, 고혈압)를 예측**하고, 위험이 감지되면 **사용자 지역(city, town) 내 '내과' 병원 5곳을 자동 추천**합니다.  
추천 정보는 **병원명, 주소, 전화번호, 위도·경도까지 포함한 맞춤형 의료 정보**로 제공되며, **CSV 기반 병원 데이터와 연동된 위치 정보 시각화**도 함께 지원합니다.
